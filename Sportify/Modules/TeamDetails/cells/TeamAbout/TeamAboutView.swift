...
class TeamAboutView: UIView {
    ...
    private func setupUI() {
        ...
        containerCard.backgroundColor =
            UIColor(named: "Card_Background")
            ?? UIColor(red: 0.10, green: 0.13, blue: 0.25, alpha: 1)
        containerCard.layer.cornerRadius = 16
        containerCard.layer.masksToBounds = true

        titleLabel.text = NSLocalizedString("about_team_title", comment: "")
        titleLabel.textColor = UIColor(named: "Text_Color") ?? .white
        titleLabel.font = UIFont.boldSystemFont(ofSize: 18)

        descriptionLabel.textColor =
            UIColor(named: "Secondary_Text")
            ?? UIColor(red: 0.63, green: 0.68, blue: 0.75, alpha: 1)
