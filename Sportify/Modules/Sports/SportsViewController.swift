...
class SportsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let localizedSportsTitle = NSLocalizedString(
            "sports_title", comment: "Title for sports screen")
        setupAppNavigationBar(withTitle: localizedSportsTitle)

        // 2. Localize the Tab Bar Item Title
        self.tabBarItem.title = localizedSportsTitle
        setupSettingsButton()
...
extension SportsViewController: SportsViewProtocol {
    ...
    func navigateToLeagues(with sport: Sport) {
        ...
        if let leaguesVC = storyboard.instantiateViewController(
            withIdentifier: "LeaguesViewController") as? LeaguesViewController
        {
    ...
    func collectionView(
        ...
        let cell =
            collectionView.dequeueReusableCell(
                withReuseIdentifier: "HomeSportsCell", for: indexPath)
